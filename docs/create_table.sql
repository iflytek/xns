...
create table t_group (id uuid primary key, create_at int, update_at int, description text, name text unique, idc_id uuid, healthy_check_mode text, healthy_check_config text, healthy_num int, un_healthy_num int, healthy_interval int, unhealthy_interval int, lb_mode text, lb_config text, server_tags text, ip_alloc_num int, default_servers text, port int, CONSTRAINT idc_id_fk FOREIGN KEY  (idc_id)  REFERENCES t_idc (id));
...
create table t_domains (host text, group_n text,tags text ,primary  key  (host,group_n));
